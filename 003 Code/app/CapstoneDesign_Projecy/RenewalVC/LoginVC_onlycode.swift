...
//  LoginProject
//
//  Created by 김종훈 on 2023/04/05.
//

import UIKit

final class LoginVC_onlycode: UIViewController {
    ...
    private func configure() {
        view.backgroundColor = #colorLiteral(red: 0.7319982737, green: 1, blue: 0.6551536722, alpha: 1)
    ...
    @objc func loginButtonTapped() {
        ...
        guard let nextVC = self.storyboard?.instantiateViewController(identifier: "MainVC") as? MainVC else { return }
...
extension LoginVC_onlycode: UITextFieldDelegate {
    // MARK: - 텍스트필드 편집 시작할때의 설정 - 문구가 위로올라가면서 크기 작아지고, 오토레이아웃 업데이트
    func textFieldDidBeginEditing(_ textField: UITextField) {
        
        if textField == emailTextField {
            emailTextFieldView.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
        ...
        if textField == passwordTextField {
            passwordTextFieldView.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
